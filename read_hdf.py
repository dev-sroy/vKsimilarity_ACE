...
for y in range(2001,2009):
  ...
  df = pd.DataFrame(columns = ['Datetime', 'Proton density', 'Proton speed', 'x_dot_RTN','y_dot_RTN','z_dot_RTN'])
  ...
  for i in range(len(vel_arr)):
    ...
    if day==vel_arr[i-1][1] or i==0:
      ...
      outfile = r'C:\Users\Sohom Roy\Dropbox\My PC (LAPTOP-SLR6UBDE)\Desktop\Research\ACE data\Velocity data_64s//'+convert_date(dtnew)+'.csv'
