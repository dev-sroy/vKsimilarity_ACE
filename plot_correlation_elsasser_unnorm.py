# -*- coding: utf-8 -*-
"""
Created on Fri May 14 14:03:55 2021
...
for f in os.listdir(dir):
  if f not in ex_f:
    ...
    interp_df = pd.DataFrame(columns = ['Lag_spatial', 'z+_tr(unnormalized)', 'z-_tr(unnormalized)'])
    ...
    if not os.path.isdir(fname):
      ...
      if vmag>=500:
        try:
          ...
          axs[1].plot(df['Lag_spatial'], df['z-_tr(unnormalized)'], color='peru', alpha = 0.2)
          
          intf = interp1d(df['Lag_spatial'], df['z+_tr(unnormalized)'], bounds_error = False)
          intf2 = interp1d(df['Lag_spatial'], df['z-_tr(unnormalized)'], bounds_error = False)
          
          interp_df['Lag_spatial'] = lag
          interp_df['z+_tr(unnormalized)'] = intf(lag)
          interp_df['z-_tr(unnormalized)'] = intf2(lag)
...
zplus_mean = corr_interp_df.groupby(['Lag_spatial'])['z+_tr(unnormalized)'].mean()
zminus_mean = corr_interp_df.groupby(['Lag_spatial'])['z-_tr(unnormalized)'].mean()

zplus_std = corr_interp_df.groupby(['Lag_spatial'])['z+_tr(unnormalized)'].std()
zminus_std = corr_interp_df.groupby(['Lag_spatial'])['z-_tr(unnormalized)'].std()
...
axs[0].set_ylabel(r'$R_{z^+}(r)$', fontsize = 80)
axs[1].set_ylabel(r'$R_{z^-}(r)$', fontsize = 80)
...
plt.savefig(r"C:\Users\Sohom Roy\Dropbox\My PC (LAPTOP-SLR6UBDE)\Desktop\Research\Plots\corr_zplus_zminus_unnorm_fast_1e7.pdf")
