# -*- coding: utf-8 -*-
"""
Created on Fri Jun  4 15:32:33 2021
  ...
  try:
    if f not in ex_f:
      ...
      if not os.path.isdir(fname):
        ...
        if vmag>=500:
          plt.plot(df['Lag_z'], df['S2_z'], color='peru', alpha = 0.2)
...
z_mean = corr_interp_df.groupby(['Lag_z'])['S2_z'].mean()
z_std = corr_interp_df.groupby(['Lag_z'])['S2_z'].std()
...
# axs[0].set_xlim(0.07, 1.3)
# axs[1].set_xlim(0.07, 1.3)
...
cond2 = np.logical_and(np.logical_and(z_mean<1, z_mean>0.1), lag>0.2)
#cond3 = np.logical_and(np.logical_and(zminus_mean<1, zminus_mean>0.1), lag>0.2)

p = np.polyfit(np.log10(lag[cond2]), np.log10(z_mean[cond2]), 1)
#p2 = np.polyfit(np.log10(lag[cond3]), np.log10(zminus_mean[cond3]), 1)

plt.plot(lag, z_mean, 'k', linewidth = 2.0, zorder = 25, label = 'Mean, slope = ' + str(round(p[0], 2)))
plt.fill_between(lag[cond], (z_mean - z_std)[cond], (z_mean + z_std)[cond], color = 'blue', alpha = 0.5, zorder = 10, label = r'$1\sigma$')
plt.fill_between(lag[cond], (z_mean - 2*z_std)[cond], (z_mean + 2*z_std)[cond], color = 'dodgerblue', alpha = 0.5, zorder = 20, label = r'$2\sigma$')
...
plt.ylabel(r'$\hat{S}^{(2)}_{z}(r/\lambda)$', fontsize = 80)
# 
...
plt.legend(fontsize = 80, loc = 'best')
...
plt.savefig(r"C:\Users\Sohom Roy\Dropbox\My PC (LAPTOP-SLR6UBDE)\Desktop\Research\Plots\sf2_zplus_zminus_1norm_fast_1e7_loglog_mod.pdf")