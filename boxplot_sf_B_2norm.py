  ...
  if f!='corr_lengths_elsasser.txt':
    try:
      ...
      if vmag >= 500:
        ...
        plt.plot(df['Lag_2norm_B'], df['B_sf'], color ='peru', alpha = 0.2)
...
corr_interp_df.boxplot(column = 'B_sf', by = 'Lag_2norm_B', ax = axs, showfliers = False, positions = lag, widths = 0.015, medianprops = {'linewidth' : 5.0, 'color' : 'k'}, patch_artist = True, boxprops = {'facecolor' : 'blue', 'alpha' : 0.2})
#plt.xlim(1e-2, 1.0)
#plt.ylim(1e-2, 10)
plt.xlim(0, 10.0)
plt.ylim(0, 4.0)
